...
class operatorPlugin(Plugin):
    ...
    def __init__(self, context):
        ...
        self.pub = rospy.Publisher("operator_to_bhavya",String,queue_size=10);
        self.sub = rospy.Subscriber("menu_num_pub", numpy_msg(Floats), self._callback)
        self.sub2 = rospy.Subscriber("PANDU_to_operator",String, self._callback2)
    ...
    def updateTableStat(self, st):
        ...
        if st == 'O':
            subs = "Item 1   :" + str(self.inData[0])
            subs2 = "\nItem 2  :" + str(self.inData[1])
            subs3 = "\nItem 3  :" + str(self.inData[2])
    ...
    def _deliver_t1(self):
        tempS = 'D'+str(rospy.get_param('/TableNum'))
        self.pub.publish(tempS)
        self.clearing.emit()
        pass
